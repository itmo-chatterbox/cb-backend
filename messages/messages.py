    ...
    return (
        ...
        .where(
            (
                ...
                & (Message.user_reciever == collocutor)
            )
            | (
                (Message.user_sender == collocutor)
                & (Message.user_reciever == current_user)
            )
        )
        .order_by(Message.sending_date.asc())
    )
...
def get_chats(user: User):
    query_sender = (
        Message.select(User)
        .where(Message.user_sender == user)
        .join(User, on=(User.id == Message.user_reciever))
        .distinct()
    )

    query_receiver = (
        Message.select(User)
        .where(Message.user_reciever == user)
        .join(User, on=(User.id == Message.user_sender))
        .distinct()
    )
...
async def my_chats(data: SendMessageDTO, session_data: SessionData = Depends(verifier)):
    ...
    Message.create(
        ...
        user_reciever=collocutor,
        sending_date=datetime.datetime.now().timestamp(),
        text=data.text,
    )
